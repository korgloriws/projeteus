...
import { formatDatePtBR, formatDateTimePtBR } from "@/lib/dates";
...
export default function ProjectDetail() {
  ...
  const projectOrgIds = new Set([...empresaOrgIds, ...entePublicoOrgIds]);
  const empresaOrgIdSet = new Set(empresaOrgIds);
  const entePublicoOrgIdSet = new Set(entePublicoOrgIds);
  const tasksInReview = tasks.filter((task) => task.status === "em_revisao").length;
  ...
  const canValidateDelivery =
    me?.role === "admin" ||
    (Boolean(me?.organizationId) &&
      entePublicoOrgIdSet.has(me!.organizationId!) &&
      Boolean(members?.some((member) => member.userId === me?.id)));
  ...
  function handleSubmitTaskForValidation(task: Task) {
    updateTask.mutate(
      { id: task.id, data: { status: "em_revisao" } },
      {
        onSuccess: () => {
          invalidateProjectQueries();
          toast({
            title: "Enviada para validação",
            description: `${task.title} aguarda validação do ente público.`,
          });
        },
        onError: (err) => {
          toast({
            variant: "destructive",
            title: "Erro ao enviar para validação",
            description:
              err instanceof Error
                ? err.message
  ...
  function handleApproveTask(task: Task) {
    const fromValidation = isAwaitingValidation(task.status);
    updateTask.mutate(
      { id: task.id, data: { status: "concluida" } },
      {
        onSuccess: () => {
          invalidateProjectQueries();
          toast({
            title: fromValidation ? "Entrega aprovada" : "Tarefa concluída",
            description: fromValidation
              ? `${task.title} foi aprovada pelo ente público.`
        ...
        onError: (err) => {
          toast({
            variant: "destructive",
            title: fromValidation ? "Erro ao aprovar" : "Erro ao concluir",
            description:
              err instanceof Error
                ? err.message
  ...
  function handleReturnTask(task: Task) {
    updateTask.mutate(
      { id: task.id, data: { status: "em_andamento" } },
      {
        onSuccess: () => {
          invalidateProjectQueries();
          toast({
            title: "Entrega devolvida",
            description: `${task.title} voltou para a empresa ajustar.`,
          });
        },
        onError: (err) => {
          toast({
            ...
            description:
              err instanceof Error
                ? err.message
                : "Não foi possível devolver a tarefa.",
          });
        },
      },
    );
    ...
    if (editingStage && stageStatus === "concluido") {
      ...
      if (!canCompleteStage) {
        const message = stageTasks.length === 0
          ...
          : `Todas as tarefas precisam estar aprovadas pelo ente antes de finalizar a etapa. ${pendingTasks.length} tarefa(s) pendente(s).`;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-col items-start gap-2 text-sm text-muted-foreground sm:items-end shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              Criado em: {formatDatePtBR(project.createdAt)}
            </div>
            {project.dueDate && (
              <div className="flex items-center gap-2 text-foreground font-medium">
                <AlertCircle className="h-4 w-4 text-orange-500" />
                Prazo: {formatDatePtBR(project.dueDate)}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t">
          <ProjectDualPartyPanel
            empresaOrgIds={empresaOrgIds}
            entePublicoOrgIds={entePublicoOrgIds}
                          ...
                          <AlertDescription>
                            {!hasTasks ? (
                              ...
                              <div className="space-y-1">
                                <p>
                                  Todas as tarefas precisam estar{" "}
                                  <strong>aprovadas</strong> pelo ente antes de
                                  finalizar a etapa.{" "}
                            ...
                            return (
                              ...
                              <div className="p-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between hover:bg-muted/30 transition-colors">
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  {task.dueDate && (
                                    <div className="text-xs text-muted-foreground mt-1">
                                      Prazo: {formatDatePtBR(task.dueDate)}
                                    </div>
                                  )}
                                </div>
                                </div>
                                <div className="flex flex-wrap items-center gap-2 sm:justify-end sm:shrink-0">
                                  <Badge
                                    variant="outline"
                                    className={`text-[10px] ${getTaskStatusColor(task.status)}`}
                                  >
                                  ...
                                  {canValidateDelivery &&
                                  isAwaitingValidation(task.status) ? (
                                    <>
                                      <Button
                                        size="sm"
                                        variant="outline"
                                        className="h-7 text-xs text-green-700 border-green-200 hover:bg-green-50 dark:hover:bg-green-950/30"
                                  ...
                                  {canValidateDelivery &&
                                  canManageTasks &&
                                  !isApprovedDelivery(task.status) &&
                                  !isAwaitingValidation(task.status) ? (
                                    <Button
                                      size="sm"
                                      variant="outline"
                                  ...
                                  !isAwaitingValidation(task.status) ? (
                                    <Button
                                      size="sm"
                                      variant="outline"
                                      className="h-7 text-xs text-amber-800 border-amber-200 hover:bg-amber-50 dark:text-amber-300 dark:border-amber-500/30 dark:hover:bg-amber-950/30"
                                      onClick={() =>
                                        handleSubmitTaskForValidation(task)
                                      }
                                      onPointerDown={stopDragPointer}
                                      disabled={updateTask.isPending}
                                    >
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <PartySideBadge
                            side={resolvePartySide(
                              member.user.organizationId,
                              empresaOrgIdSet,
                              entePublicoOrgIdSet,
                            )}
                          />
                    ...
                    <div className={`flex justify-between items-start gap-2 mb-1 ${isOwnMessage ? "flex-row-reverse" : ""}`}>
                      ...
                      <div className={`flex items-center gap-2 ${isOwnMessage ? "flex-row-reverse" : ""}`}>
                        ...
                        <span className="text-[10px] text-muted-foreground whitespace-nowrap">
                          {formatDateTimePtBR(comment.createdAt)}
                        </span>
                        {canDeleteComment && (
                          <Button
                            size="sm"
                  ...
                  <Select value={taskStatus} onValueChange={(value) => setTaskStatus(value as typeof taskStatus)}>
                    ...
                    <SelectContent>
                      ...
                      {(canValidateDelivery ||
                        ...
                        <SelectItem value="concluida">Aprovada</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                  <p className="text-[11px] text-muted-foreground">
                    {canValidateDelivery
                      ? "Como ente público, você pode concluir direto — sem auto-validação."
                      : "Fluxo da empresa: envie para validação → o ente aprova ou devolve."}
                  </p>
                </div>
                <div className="space-y-2">
