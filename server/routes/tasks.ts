...
router.patch("/tasks/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const canValidate = await canValidateTaskDelivery(req.appUser!, project);
  const nextStatus = parsed.data.status ?? task.status;
  const statusChanging =
    parsed.data.status !== undefined && parsed.data.status !== task.status;

  const bodyKeys = Object.keys(parsed.data);
  const hasAssigneeExtra = assigneeIdsSchema.safeParse(req.body).success;
  const isEnteValidationStatusChange =
    statusChanging &&
    !hasAssigneeExtra &&
  ...
  if (!canManage && !(canValidate && isEnteValidationStatusChange)) {
    res.status(403).json({ error: "Forbidden" });
    return;
  }

  if (statusChanging) {
    const transition = await assertTaskStatusTransition({
      actor: req.appUser!,
      project,
  ...
  // Validadores do ente só podem alterar o status (aprovar/devolver).
  if (!canManage && canValidate) {
    if (bodyKeys.some((key) => key !== "status") || hasAssigneeExtra) {
      res.status(403).json({
        error: "O ente público só pode aprovar ou devolver a entrega nesta ação.",
      });
      return;
    }
  }
  ...
  const submittedForValidation =
    updated.status === "em_revisao" && previousStatus !== "em_revisao";
  const returnedFromValidation =
    previousStatus === "em_revisao" &&
    (updated.status === "em_andamento" || updated.status === "a_fazer");
  ...
  if (submittedForValidation) {
    await runNotify(
      createNotificationsForUsers(memberIds, {
        actorUserId: actorId,
        type: "task_submitted_for_validation",
        title: "Entrega aguardando validação",
        message: `${actorName} enviou a tarefa "${updated.title}" para validação do ente público.`,
        projectId: project.id,
        stageId: updated.stageId,
        taskId: updated.id,
        link,
    ...
    const approvedAfterValidation = previousStatus === "em_revisao";
    await runNotify(
      createNotificationsForUsers(memberIds, {
        actorUserId: actorId,
        type: approvedAfterValidation ? "task_approved" : "task_completed",
        title: approvedAfterValidation
          ? "Entrega aprovada"
          : "Tarefa concluída",
        message: approvedAfterValidation
          ? `${actorName} aprovou a tarefa "${updated.title}".`
  ...
  } else if (returnedFromValidation) {
    await runNotify(
      createNotificationsForUsers(memberIds, {
        actorUserId: actorId,
        type: "task_returned",
        title: "Entrega devolvida",
        message: `${actorName} devolveu a tarefa "${updated.title}" para ajustes.`,
        projectId: project.id,
        stageId: updated.stageId,
        taskId: updated.id,
        link,
